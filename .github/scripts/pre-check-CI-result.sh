...
response=$(curl -s  -H "Content-Type: application/json" \
                    ...
                    -d "{\"type\": \"RETRIEVE-TASK-STATUS\", \"aone\": { \"projectId\": \"${PROJECT_ID}\", \"pipelineId\": \"${PIPELINE_ID}\"}, \"commitId\": \"${COMMIT_ID}\"}" "https://get-tasback-pre-aiffqmsbgj.cn-hangzhou.fcapp.run")
...
if [[ "$main_status" == "DONE" ]]; then
    ...
    exit 0
elif [[ "$main_status" == "PENDING" || "$main_status" == "RUNNING" ]]; then
    echo "CI is currently ${main_status} for this commit"
    echo "Need to wait or run CI"
    exit 1
elif [[ "$main_status" == "FAILED" ]]; then
    echo "Previous CI failed for this commit"
    echo "Need to run CI again"
    exit 1
else
    echo "CI status is ${main_status} for this commit"
    echo "Need to run CI"
    exit 1
fi
